...
- [LlamaIndex integration](/integrations/llamaindex) — `PDFMarkdownReader` for structured per-page document loading
- [Page selection & chunking](/guides/page-chunking) — chunking strategies and token budgeting in detail
- [Image extraction](/guides/image-extraction) — extracting images for multimodal model input
- [PdfExtractor API reference](/api-reference/pdf-extractor) — complete method signatures and parameters