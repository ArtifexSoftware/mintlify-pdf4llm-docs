...
<ParamField path="ocrLanguage" type="string" default='"eng"'>
  Tesseract language code. Use `+` to combine multiple languages: `"eng+fra"`. The corresponding language data files must be installed. Only relevant when `useOcr` or `forceOcr` is `true`. See [Tesseract Language Packs](/guides/tesseract-language-packs).
</ParamField>
...
`string` — A JSON array, one object per processed page. See [JSON layout export](/guides/json-layout) for the full schema.
...
`ParsedDocument` — A typed .NET object containing one `ParsedPage` per processed page. See [ParsedDocument reference](/api-reference/parsed-document) for the full type hierarchy.
...
`PDFMarkdownReader` — Call `.LoadData()` on the result to load a PDF. See [PDFMarkdownReader reference](/api-reference/pdf-markdown-reader) for full documentation.
