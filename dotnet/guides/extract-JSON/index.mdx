...
foreach (JObject page in pages)
{
	...
	foreach (JObject box in (JArray?)page["boxes"] ?? new JArray())
	{
...
foreach (JObject page in pages)
{
    foreach (JObject box in (JArray?)page["boxes"] ?? new JArray())
    {
        if (box["type"]!.Value<string>() != "text") continue;

        foreach (JObject line in (JArray?)box["textlines"] ?? new JArray())
        {
            foreach (JObject span in (JArray?)line["spans"] ?? new JArray())
            {
