---
title: Extract JSON
description: Use [ToJson()](/dotnet/api/to-json) to get bounding boxes, layout data, and structured page content for custom pipelines.
icon: "brackets-curly"
---
...
The return value is a JSON array — one object per processed page. See the [JSON layout export guide](/guides/json-layout) for a full field reference.
...
As with [`ToMarkdown()`](/api-reference/to-markdown), you can limit extraction to specific pages:
...
<Note>
  For the full API signature, see the [ToJson() API reference](/dotnet/api/to-json).
</Note>
...
<CardGroup cols={2}>
  <Card title="JSON Schema" icon="file-code" href="/dotnet/guides/json-layout">
