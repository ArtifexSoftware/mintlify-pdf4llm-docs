...
<ResponseField name="list[dict]" type="list">
  ...
  | `metadata` | `dict` | Page metadata — see [Chunk Schema](/python/reference/chunk-schema) for the full schema |
</ResponseField>
...
| `ImportError` | [`use_layout(True)`](/python/api/use_layout) but the `layout` dependency is not installed |
